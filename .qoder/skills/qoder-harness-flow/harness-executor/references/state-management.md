...
状态值：`in_progress`、`completed`、`failed`、`blocked`、`abandoned`。

### task.json vs tasks.md — 不要混淆

这两个文件名相似但职责完全不同：

| 文件 | 格式 | 路径 | 作用 | 创建者 |
|------|------|------|------|--------|
| **task.json** | JSON | `harness/tasks/<id>/state/task.json` | 状态机核心，记录任务元数据和执行状态 | `task_state.py init` |
| **tasks.md** | Markdown | `docs/exec-specs/<slug>/tasks.md` | 执行计划，记录 Task 1/2/3 的具体步骤 | harness-spec Step 4 |

**关联方式**：task.json 的 `plan_path` 字段指向 tasks.md，将状态文件与计划文件串联。

```
task.json.plan_path ──指向──→ docs/exec-specs/<slug>/tasks.md
task.json.spec_path ──指向──→ docs/exec-specs/<slug>/   （目录，含 spec.md + tasks.md + acceptance.md）
```

**门禁检查对象**：`validate_plan_gate` 读 task.json 的 `plan_path` → 找到 tasks.md → 验证计划内容完整性（四段式结构）。`validate_completion_gate` 读 task.json 的状态 + verification-report.json → 决定是否允许 complete。
