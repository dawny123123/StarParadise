...
用于 0→1 项目脚手架的完整、即用型模板。每个部分提供真实的文件内容——不是占位符——用于引导具有完整 harness 基础设施的新项目。
...
使用 `references/linter-templates.md` 中的模板，其中：
- `modulePath` 设置为项目的模块路径
- `layers` 配置为项目的目录结构：
  ```go
  var layers = [][]string{
...
使用 `references/linter-templates.md` 中的模板，文件大小限制设置为 500（适用于新项目，以后可以增长到 1000）。

---

## Go Web API

与 Go CLI 工具相同，有以下添加/更改：
...
与 Python CLI 相同，有以下添加：

### src/{package_name}/main.py (Web API 版本)
...
这些文件无论技术栈如何都相同。所有模板使用：
- **分层编号**（1, 1.1, 1.2...）用于稳定的交叉引用
- **占位符来源引用**（`> Sources: [

file]()`）指向将要创建的文件
- **Mermaid 图表**显示计划的架构

### AGENTS.md 模板（DeepWiki 风格）
...
### docs/ARCHITECTURE.md 模板（DeepWiki 风格, Greenfield）

对于 greenfield 项目，架构文档描述**计划的**结构，其中 Mermaid 图表显示预期的层层次结构。来源引用指向执行计划将创建的文件。
