...
- Rewards（奖励目标）：记录孩子设定的奖励目标，跟踪当前累计金额与达成状态。
- Transactions（交易记录）：记录金钱类收支流水（earn/spend）。
...
- Rewards
  - 主键：id；外键：child_id → Children(id)
  - 字段：title、target_amount、current_amount、is_achieved、created_at
  - 约束：默认未达成；默认累计金额
- Transactions
  - 主键：id；外键：child_id → Children(id)
  - 字段：type（earn/spend）、amount、description、created_at
  - 约束：默认时间
- Points
  - 主键：id；外键：child_id → Children(id)
...
- 其他约束
  ...
  - Transactions.type：限定 earn/spend 类型
...
### 触发器与存储过程
- 当前实现未使用触发器或存储过程
- 业务逻辑集中在路由层（如 checkins 流程中的事务封装）
...
- 奖励目标：rewards.js 支持按 child_id 过滤、创建、更新、删除
...
- [rewards.js:5-84](file://star-park/server/src/routes/rewards.js#L5-L84)
...
- 写入优化
  - 使用事务批量写入（如 checkins、points 的事务封装）
  - 控制单次事务大小，避免长时间锁持有
- 缓存策略
...
## 结论
本设计以 SQLite 为核心，围绕 Children、Tasks、Checkins、Rewards、Transactions、Points 六张表构建家庭激励系统。通过外键约束与事务封装保障数据一致性，利用 WAL 模式与索引建议提升性能。当前未使用触发器与存储过程，业务逻辑集中于路由层，便于维护与扩展。建议后续引入复合索引与缓存策略，持续通过一致性检查脚本保障 DDL 与代码的一致性。

## 附录

### 数据迁移路径与版本管理策略
- 迁移策略
  - 通过 ALTER TABLE 动态添加字段（如 children.points_balance），兼容已有数据
  - 使用 try/catch 忽略重复添加字段的异常
- 版本管理
