...
## 产品概述
本章节聚焦“星星乐园”项目的持续集成与持续交付（CI/CD）及自动化部署能力。该体系以云效 Flow 流水线为核心，将代码检查、单元测试、构建打包、主机部署与健康检查串联为可追溯的发布流程；通过版本目录 + 原子软链切换实现零停机回滚，配合 SQLite 数据库备份与端口守卫，保障生产环境稳定上线。面向产品经理与运营角色，本文强调发布节奏、质量门禁与回滚策略，避免深入技术细节。
