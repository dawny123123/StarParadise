...
## 更新摘要
**所做更改**
- 新增 app.js 文件实现更清晰的 Express.js 应用设置
- database.js 增强了数据关系支持，改进外键约束和事务处理
- 更新了项目结构图以反映新的模块化架构
- 增强了数据库访问层设计的详细说明
...
**更新** 新增了 app.js 文件实现更清晰的 Express.js 应用设置，database.js 增强了数据关系支持，提升了系统的可维护性和扩展性。

## 项目结构
后端位于 star-park/server，采用"入口 -> 应用配置 -> 中间件 -> 路由 -> 数据库"的清晰分层：
- 入口文件负责服务启动和进程管理
- app.js 集中管理 Express 应用实例、中间件注册和路由挂载
...
subgraph "后端服务架构"
A["index.js<br/>服务入口"]
B["app.js<br/>Express 应用配置"]
C["database.js<br/>SQLite 初始化"]
D["seed.js<br/>种子数据"]
E["routes/<br/>路由模块集合"]
end
A --> B
...
- [app.js:1-50](file://star-park/server/src/app.js#L1-L50)
...
**更新** app.js 文件提供了更清晰的 Express 应用配置，将中间件注册、路由挂载和错误处理集中在一个文件中，提高了代码的可维护性。

章节来源
- [index.js:16-32](file://star-park/server/src/index.js#L16-L32)
- [app.js:10-45](file://star-park/server/src/app.js#L10-L45)
...
## 架构总览
后端采用"入口 -> 应用配置 -> 中间件 -> 路由 -> 数据库"的线性处理链，app.js 作为应用配置中心统一管理中间件和路由，路由模块各自封装业务逻辑，数据库模块提供统一连接与建模，形成清晰的职责边界与可演进的扩展点。
...
- 统一错误处理：app.js 中集中管理全局错误处理逻辑

**更新** 通过 app.js 实现了更集中的中间件管理和错误处理机制，提高了系统的健壮性。
...
**图表来源**
- [index.js:16-32](file://star-park/server/src/index.js#L16-L32)
- [app.js:20-45](file://star-park/server/src/app.js#L20-L45)
...
**更新** database.js 增强了数据关系支持，改进了外键约束处理和事务管理机制，提升了数据一致性和完整性。
...
**新增** app.js 文件作为应用配置中心，将原本分散在 index.js 中的配置逻辑集中管理，提高了代码的组织性和可维护性。
...
participant App as "app.js 应用配置"
...
- 集中配置：通过 app.js 统一管理路由挂载和中间件配置

**更新** 引入了 app.js 作为配置中心，进一步简化了路由组织的复杂度，使微服务化架构更加清晰。

章节来源
- [ARCHITECTURE.md:86-93](file://docs/ARCHITECTURE.md#L86-L93)
- [app.js:25-45](file://star-park/server/src/app.js#L25-L45)
...
- [app.js:1-20](file://star-park/server/src/app.js#L1-L20)
...
- 代码层面
  ...
  - app.js 集中管理中间件，减少重复配置开销
- 运行层面
  - 生产环境建议使用进程管理器与反向代理
  - 日志与监控可按需扩展，当前为控制台输出
  - 内存使用和垃圾回收优化

**更新** 通过 app.js 的集中配置管理和 database.js 的性能优化，整体系统性能得到进一步提升。
...
- [app.js:10-30](file://star-park/server/src/app.js#L10-L30)
...
- 排查步骤
  ...
  - 核对请求参数与数据库表结构
  - 查看服务日志定位异常堆栈
  - 检查 app.js 中的中间件配置是否正确
- 种子数据
  - 若数据库为空，服务启动时会自动初始化演示数据
- 常见问题诊断
  - 端口冲突：确认 3001 端口未被占用
  - 数据库文件权限：确保 data 目录有写权限
  - 依赖安装：检查 node_modules 是否完整

**更新** 增加了针对 app.js 配置问题的排查指导，帮助用户更快定位和解决配置相关问题。

章节来源
- [server.md:103-110](file://docs/design-docs/server.md#L103-L110)
- [seed.js:3-42](file://star-park/server/src/seed.js#L3-L42)
- [app.js:35-50](file://star-park/server/src/app.js#L35-L50)

## 结论
该后端架构以 Express + better-sqlite3 为核心，通过 app.js 集中管理应用配置，配合模块化路由实现业务域高内聚、低耦合，配合 WAL 与事务保障数据一致性。整体设计简洁、易维护、易扩展，适合家庭场景的小型应用。未来可在监控、鉴权与缓存方面进一步增强，以支撑更大规模的使用场景。

**更新** 新增的 app.js 文件和增强的 database.js 显著提升了系统的可维护性和扩展性，为后续功能扩展奠定了良好基础。

## 附录
- 启动与开发
  - 后端启动：cd star-park/server && npm start 或 npm run dev
  - 开发代理：管理后台开发时需配置 /api 代理至后端 3001 端口
  - 应用配置：通过 app.js 统一管理中间件和路由配置
- API 参考
  - 基础信息、接口列表与错误响应详见 API 文档
- 架构演进
  - 从 index.js 单文件管理到 app.js 集中配置的演进过程
  - 数据库层的持续优化和改进计划

**更新** 增加了关于 app.js 配置管理和架构演进的相关说明。
