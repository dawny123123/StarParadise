...
- [pages/wallet/index.vue](file://star-park/miniprogram/src/pages/wallet/index.vue)
- [pages/rewards/index.vue](file://star-park/miniprogram/src/pages/rewards/index.vue)
...
## 更新摘要
**变更内容**
- 新增钱包页面（wallet）实现，支持多孩子余额查看、交易明细和积分记录
- 新增奖励目标页面（rewards）实现，支持进度追踪和目标管理
- 新增积分弹窗组件（PointsModal），支持特殊积分发放功能
- 完善API客户端封装，新增积分管理和交易记录相关接口
- 扩展TabBar配置，新增钱包入口
- 增强多孩子支持，所有页面均支持孩子切换功能
...
H --> H1["ChildCard.vue"]
H --> H2["TaskItem.vue"]
H --> H3["PointsModal.vue"]
A --> I["pages/"]
I --> I1["index/index.vue"]
I --> I2["checkin/index.vue"]
I --> I3["records/index.vue"]
I --> I4["wallet/index.vue"]
I --> I5["rewards/index.vue"]
I --> I6["profile/index.vue"]
...
- [api/index.js:1-78](file://star-park/miniprogram/src/api/index.js#L1-L78)
...
- [pages/wallet/index.vue:1-368](file://star-park/miniprogram/src/pages/wallet/index.vue#L1-L368)
- [pages/rewards/index.vue:1-320](file://star-park/miniprogram/src/pages/rewards/index.vue#L1-L320)
- [pages/profile/index.vue:1-211](file://star-park/miniprogram/src/pages/profile/index.vue#L1-L211)
- [package.json:1-29](file://star-park/miniprogram/package.json#L1-L29)
...
- PointsModal 组件
  - **新增** 积分发放弹窗组件，支持选择孩子、输入积分数值和理由，提供完整的积分发放流程。
...
#### 钱包页(wallet)
- **新增** 功能概览
  - 孩子选择器、余额双卡片（金额余额和积分余额）、交易明细与积分记录Tab切换。
- 交互流程
  - 选择孩子 -> 加载余额信息 -> 切换Tab查看交易明细或积分记录 -> 格式化时间显示。
- 数据流
  - api.getChildren 初始化孩子列表；api.getTransactions 获取交易记录；api.getPoints 获取积分记录。

```mermaid
flowchart TD
Start(["进入钱包页"]) --> Init["初始化孩子列表"]
Init --> SelectChild{"选择孩子"}
SelectChild --> LoadBalance["加载余额信息"]
LoadBalance --> ShowTabs["显示Tab切换"]
ShowTabs --> Transactions{"查看交易明细"}
ShowTabs --> PointsRecords{"查看积分记录"}
Transactions --> FormatTime["格式化时间显示"]
PointsRecords --> FormatTime
FormatTime --> Display["展示记录列表"]
Display --> End(["结束"])
```

图示来源
- [pages/wallet/index.vue:117-182](file://star-park/miniprogram/src/pages/wallet/index.vue#L117-L182)
- [api/index.js:69-76](file://star-park/miniprogram/src/api/index.js#L69-L76)
...
#### 奖励目标页(rewards)
- **新增** 功能概览
  - 孩子选择器、奖励目标列表、进度条展示、达成状态标识与统计信息。
- 交互流程
  - 选择孩子 -> 加载奖励目标 -> 计算进度百分比 -> 显示达成状态 -> 统计进行中/已达成/已兑换数量。
- 数据流
  - api.getChildren 初始化孩子列表；api.getRewards 获取奖励目标数据。

```mermaid
flowchart TD
Start(["进入奖励目标页"]) --> LoadData["加载孩子与奖励数据"]
LoadData --> FilterRewards["筛选当前孩子的奖励"]
FilterRewards --> CalculateProgress["计算进度百分比"]
CalculateProgress --> CheckStatus["检查达成状态"]
CheckStatus --> DisplayList["显示奖励列表"]
DisplayStats["显示统计信息"] --> End(["结束"])
```

图示来源
- [pages/rewards/index.vue:103-127](file://star-park/miniprogram/src/pages/rewards/index.vue#L103-L127)
- [api/index.js:54-61](file://star-park/miniprogram/src/api/index.js#L54-L61)
...
CompPM["components/PointsModal.vue"] --> Style
PageIdx["pages/index/index.vue"] --> CompCC
PageChk["pages/checkin/index.vue"] --> CompTI
PageWallet["pages/wallet/index.vue"] --> CompPM
PageRewards["pages/rewards/index.vue"] --> CompPM
```
...
- 钱包数据加载失败
  - **新增** 现象：钱包页面显示空状态或数据不正确。
  - 排查：检查 api.getTransactions 和 api.getPoints 接口返回；确认孩子ID传递正确；查看控制台错误日志。
- 奖励目标进度异常
  - **新增** 现象：进度条显示不正确或状态标识异常。
  - 排查：检查 getProgressPercent 计算逻辑；确认 current_amount 和 target_amount 数据格式；验证 is_achieved 状态。
...
- [pages/wallet/index.vue:117-182](file://star-park/miniprogram/src/pages/wallet/index.vue#L117-L182)
- [pages/rewards/index.vue:93-97](file://star-park/miniprogram/src/pages/rewards/index.vue#L93-L97)

## 结论
本项目以 uni-app 为核心，结合 Vue 3 Composition API 与 SCSS 变量体系，实现了清晰的页面结构、可复用的组件系统与统一的网络请求封装。通过 pages.json 与 manifest.json 的配置，兼顾了微信小程序与 H5 的跨平台需求。新增的钱包和奖励目标页面进一步完善了家庭激励系统的移动端体验，提供了实时奖励计算和多孩子支持。建议在后续迭代中进一步完善缓存策略、错误监控与埋点上报，持续优化首屏性能与交互体验。
