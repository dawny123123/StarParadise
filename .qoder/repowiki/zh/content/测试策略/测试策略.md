...
- [star-park/server/vitest.config.js](file://star-park/server/vitest.config.js)
- [star-park/server/tests/setup.js](file://star-park/server/tests/setup.js)
- [star-park/server/tests/helpers.js](file://star-park/server/tests/helpers.js)
- [star-park/server/tests/checkins.test.js](file://star-park/server/tests/checkins.test.js)
- [star-park/server/tests/children.test.js](file://star-park/server/tests/children.test.js)
- [star-park/server/tests/points.test.js](file://star-park/server/tests/points.test.js)
- [star-park/server/tests/rewards.test.js](file://star-park/server/tests/rewards.test.js)
- [star-park/server/tests/stats.test.js](file://star-park/server/tests/stats.test.js)
- [star-park/server/tests/tasks.test.js](file://star-park/server/tests/tasks.test.js)
- [star-park/server/tests/seed.test.js](file://star-park/server/tests/seed.test.js)
</cite>

## 更新摘要
**变更内容**   
- 新增Vitest框架配置与完整测试基础设施
- 添加多个路由处理器的单元测试覆盖（checkins、children、points、rewards、stats、tasks）
- 集成代码覆盖率报告生成机制
- 完善测试环境设置与辅助工具函数
...
**最新更新**：项目现已集成完整的Vitest测试框架，提供全面的后端路由处理器单元测试覆盖和代码覆盖率报告功能。
...
**新增**：后端服务现已集成Vitest测试框架，包含完整的测试套件和覆盖率报告生成。
...
SVitest["server/vitest.config.js"]
STests["server/tests/*"]
end
subgraph "前端 - 小程序"
...
- [star-park/server/vitest.config.js:1-50](file://star-park/server/vitest.config.js#L1-L50)
- [star-park/server/tests/children.test.js:1-100](file://star-park/server/tests/children.test.js#L1-L100)
...
- **新增**：Vitest测试框架配置，支持模块化测试、模拟数据库连接和覆盖率报告生成。
...
**更新**：新增Vitest单元测试层，提供完整的后端路由处理器测试覆盖。

```mermaid
graph TB
subgraph "单元测试层"
UT_DB["数据库层单元测试<br/>表结构/约束/迁移"]
UT_Routes["路由单元测试<br/>children/tasks/checkins/rewards/stats/points"]
UT_Vitest["Vitest框架配置<br/>测试环境与覆盖率"]
end
subgraph "集成测试层"
...
### 后端服务与路由测试策略
- 单元测试
  - 路由层：针对 children、tasks、checkins、rewards、stats、points 等路由的 GET/POST/PUT/DELETE 逻辑进行断言，覆盖参数校验、错误处理与返回码。
  - 数据层：对数据库初始化、WAL/外键设置、DDL 与迁移逻辑进行断言，确保表结构与字段存在。
  - **新增**：Vitest框架配置支持模块化测试、模拟数据库连接和覆盖率报告生成。
- 集成测试
...
**更新**：新增完整的Vitest测试套件，涵盖所有主要路由处理器的单元测试。
...
- [star-park/server/vitest.config.js:1-50](file://star-park/server/vitest.config.js#L1-L50)
- [star-park/server/tests/children.test.js:1-100](file://star-park/server/tests/children.test.js#L1-L100)
- [star-park/server/tests/checkins.test.js:1-100](file://star-park/server/tests/checkins.test.js#L1-L100)
- [star-park/server/tests/points.test.js:1-100](file://star-park/server/tests/points.test.js#L1-L100)
- [star-park/server/tests/rewards.test.js:1-100](file://star-park/server/tests/rewards.test.js#L1-L100)
- [star-park/server/tests/stats.test.js:1-100](file://star-park/server/tests/stats.test.js#L1-L100)
- [star-park/server/tests/tasks.test.js:1-100](file://star-park/server/tests/tasks.test.js#L1-L100)
...
- [star-park/pc-admin/src/components/ChildCard.vue:49-64](file://star-park/pc-admin/src/components/ChildCard.vue#L49-64)
- [star-park/pc-admin/src/components/Layout.vue:10-12](file://star-park/pc-admin/src/components/Layout.vue#L10-12)
- [star-park/miniprogram/src/pages/index/index.vue:99-132](file://star-park/miniprogram/src/pages/index/index.vue#L99-132)
- [star-park/pc-admin/src/views/Dashboard.vue:76-91](file://star-park/pc-admin/src/views/Dashboard.vue#L76-91)
...
- **新增**：Vitest测试框架配置，支持测试环境设置、模拟数据库连接和覆盖率报告生成。

**更新**：新增Vitest测试框架配置和测试辅助工具函数。
...
participant Vitest as "Vitest测试"
...
Harness->>Vitest : "运行单元测试"
Harness->>API : "执行 API 测试"
Harness->>DB : "执行数据库一致性检查"
Vitest-->>Harness : "测试结果与覆盖率"
API-->>Harness : "测试结果"
DB-->>Harness : "检查结果"
```
...
- [scripts/check-db-consistency.sh:1-45](file://scripts/check-db-consistency.sh#L1-L45)
- [star-park/server/vitest.config.js:1-50](file://star-park/server/vitest.config.js#L1-L50)
- [star-park/server/tests/setup.js:1-50](file://star-park/server/tests/setup.js#L1-L50)
- [star-park/server/tests/helpers.js:1-50](file://star-park/server/tests/helpers.js#L1-L50)
...
### Vitest测试框架配置
- 框架配置：Vitest配置文件定义了测试环境、模块解析、覆盖率报告生成等核心选项。
- 测试设置：setup.js提供全局测试环境设置，包括数据库连接模拟和测试数据初始化。
- 辅助工具：helpers.js提供通用的测试辅助函数，简化测试用例编写。
- 测试覆盖：涵盖所有主要路由处理器（children、tasks、checkins、rewards、stats、points）的单元测试。

**新增**：完整的Vitest测试基础设施，提供现代化的测试体验和覆盖率报告。

```mermaid
graph LR
VConfig["vitest.config.js"] --> VSetup["tests/setup.js"]
VConfig --> VHelpers["tests/helpers.js"]
VSetup --> VTests["tests/*.test.js"]
VHelpers --> VTests
VTests --> Coverage["覆盖率报告"]
VTests --> Results["测试结果"]
```
...
- [star-park/server/tests/stats.test.js:1-100](file://star-park/server/tests/stats.test.js#L1-L100)
- [star-park/server/tests/tasks.test.js:1-100](file://star-park/server/tests/tasks.test.js#L1-L100)
- [star-park/server/tests/seed.test.js:1-100](file://star-park/server/tests/seed.test.js#L1-L100)

## 依赖关系分析
- 后端路由依赖数据库连接与初始化种子数据；Express 中间件启用 CORS 与 JSON 解析。
- 前端页面通过 API 模块调用后端接口；组件之间通过 props 与事件通信。
- Harness 环境脚本负责服务生命周期管理与测试前置条件准备。
- **新增**：Vitest测试框架依赖测试辅助工具和模拟数据库连接。
...
Harness --> API
Harness --> DB
Vitest["Vitest框架"] --> VSetup["测试设置"]
Vitest --> VHelpers["测试辅助"]
VSetup --> VTests["测试用例"]
VHelpers --> VTests
VTests --> Coverage["覆盖率报告"]
```
...
- **新增**：测试性能优化：使用内存数据库进行测试，避免磁盘I/O开销；并行执行测试用例提升运行效率。
...
- **新增**：Vitest测试问题
  - 检查测试环境配置和依赖安装；确认数据库连接模拟正确；查看测试输出和覆盖率报告。
...
**更新**：新增的Vitest测试框架为后端路由处理器提供了全面的单元测试覆盖，配合代码覆盖率报告生成功能，进一步提升了项目的测试质量和开发效率。

## 附录

### 测试金字塔实施清单
- 单元测试
  - 后端路由：children/tasks/checkins/rewards/stats/points 的增删改查与错误分支。
  - 数据层：表结构、约束、迁移逻辑。
  - 前端组件：props 输入、事件发射、渲染断言。
  - **新增**：Vitest框架配置、测试环境设置、辅助工具函数。
- 集成测试
...
- **新增**：运行Vitest测试：在server目录下执行npm test。
- 清理环境：根据需要执行 Harness 的 teardown 脚本（如有）。

### Vitest测试使用指南
- 配置检查：确认vitest.config.js配置正确。
- 运行测试：在项目根目录执行npm test或在server目录执行npm test。
- 覆盖率报告：测试完成后自动生成coverage目录下的覆盖率报告。
- 调试测试：使用--watch模式进行实时测试调试。
